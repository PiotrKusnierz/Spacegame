...
public class FroggerApp extends Application {
    ...
    private Node spawnCar() {          // TIP!: I could add a few sprites and then use image view
        ...
        rect.setTranslateY((int)(Math.random() * 14) * 40);  //values decides on position and range of created
                                                             // enemies objects
        root.getChildren().add(rect);     // adding rectangle objects to the root node
        return rect;
    }

    private void onUpdate() {    // updates all game objects that we have, this method is executed every 0.016s
        for (Node car : cars)
            car.setTranslateX(car.getTranslateX() + Math.random() * 10);   // to move the enemies form left to right
